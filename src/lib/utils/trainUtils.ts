...
export const getTimeDiff = (timeTableRows: TimeTableRow[]) => {
    const visitedStations = timeTableRows.filter((row) => row.actualTime !== null);
...
export const getTimeDiffByStation = (timeTableRows: TimeTableRow[], stationName: string) => {
...
export const getCommercialStationArrivals = (train: TrainType) => {
    return train.timeTableRows.filter((row) => {
        return row.trainStopping && row.commercialStop === true && row.type === "ARRIVAL";
    });
};
...
export const getVisitedCommercialStations = (train: TrainType) => {
    return train.timeTableRows.filter((row) => {
        return row.actualTime !== null && row.trainStopping && row.commercialStop === true;
    });
};
...
export const getNextStation = (train: TrainType) => {
    ...
    const nextStation = commercialStations.find(
        (timeTableRow) => timeTableRow.actualTime === null && timeTableRow.type === "ARRIVAL",
    );
    return nextStation;
};
...
export const getCommercialStationDepartures = (train: TrainType) => {
    return train.timeTableRows.filter((row) => {
        return row.trainStopping && row.commercialStop === true && row.type === "DEPARTURE";
    });
};
